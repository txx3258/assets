$(function() {
	...
	initAppInfoMap(appInfoJsonData) ;
	...
	$('#btn_back').click(function(e){
		location.href= ctxH+'/gotoAdvertPlanHome.jspx?pkg='+$('#pkgNameH').val()  ;
	}) ;
...
function initAppInfoMap(appInfoJsonData){
	$.each(appInfoJsonData, function(i, appInfo){
		appInfoMap.put(appInfo.lcaId, appInfo);
	}) ;
}
...
function getAllOnlineApp(childrenType) {
	...
	var ctx = $("#ctxH").val();
	$.ajax({
		url : ctx + '/getAllOnlineApp4AdvertPlan.jspx?childrenType='
				+ childrenType,
		type : 'post',
		dataType : 'json',

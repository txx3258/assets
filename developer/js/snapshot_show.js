$.fn.portrait = function () {
    ...
    return this.each(function () {      var $box = $('> div', this).css('overflow', 'hidden'),
            $slider = $box.find('> ul'),
        ...
        if (($items.length % visible) != 0) {          $slider.append(repeat('<li class="empty" />', visible - ($items.length % visible)));           $items = $slider.find('> li');       }
        ...
        function gotoPage(page) {          var dir = page < currentPage ? -1 : 1,
                n = Math.abs(currentPage - page),
                left = singleWidth * dir * visible * n;
            $box.filter(':not(:animated)').animate({
                scrollLeft : '+=' + left
            }, 500, function () {              if (page == 0) {                  $box.scrollLeft(singleWidth * visible * pages);                   page = pages;               } else if (page > pages) {                  $box.scrollLeft(singleWidth * visible);                   // reset back to start position
                    page = 1;               }
...
$(document).ready(function () {$('.portrait').portrait();});
...
$(document).ready(function () {$('.wangPing').wangPing();});